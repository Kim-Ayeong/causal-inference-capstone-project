...
#        pdf.set_font("NanumGothic", size=12)
        ...
        # Save the PDF file
        pdf_path = "causal_inference_report.pdf"
        pdf.output(pdf_path)
        return pdf_path
...
#         st.info("Please upload the data first.")
    ...
    with st.expander("Flowchart for Causal Inference"):
        ...
        flowchart_image = Image.open('C:/Users/davie/Desktop/성진/coding/flowchart_image.jpg')
        st.image(flowchart_image, caption='Causal Inference Flowchart', use_column_width=True)    
...
#     st.markdown("<hr style='border:1px solid #ddd;'>", unsafe_allow_html=True)
    ...
    def generate_pdf_report(treat_column, outcome_column, change_flag, change_perc, p_value, stats_sign):
        try:
            pdf = FPDF()
            # ... PDF 생성 코드 ...
            return pdf_path
    ...
    if st.button("Generate PDF"):
        if result_flag:
            try:
                ...
                st.session_state['pdf_path'] = pdf_path  # 파일 경로도 저장
